...
#define OS_FW_VERSION                                                          \
  221 // Firmware version: 221 means 2.2.1
...
#define FLOWCOUNT_RT_WINDOW                                                    \
  1000 // flow count divisor (for computing real-time flow rate)
...
#if defined(ARDUINO)
#define MAX_EXT_BOARDS                                                         \
  8 // maximum number of 8-zone expanders (each 16-zone expander counts as 2)
#else
...
#define DEFAULT_TARGET_PD_VOLTAGE                                              \
  75 // default target voltage (unit: 100mV, so 75 means 7500mV ot 7.5V)
...
#define PIN_FREE_LIST                                                          \
  {                                                                            \
  } // no free GPIO pin at the moment
#define ETHER_BUFFER_SIZE 2048

#define PIN_ETHER_CS                                                           \
  16 // Ethernet CS (chip select pin) is 16 on OS 3.2 and above
...
extern unsigned char PIN_BUTTON_4;
extern unsigned char PIN_RELAY_1;
extern unsigned char PIN_RELAY_2;
extern unsigned char PIN_RELAY_3;
extern unsigned char PIN_RELAY_4;
extern unsigned char PIN_RELAY_5;
extern unsigned char PIN_LED;
...
#define PIN_FREE_LIST                                                          \
  {5, 6, 7, 8, 9, 11, 12, 13, 16, 19, 20, 21, 23, 25, 26} // free GPIO pins
