...
!!! note "SMTP security"
    SMTP connections are encrypted when TLS is used, but the firmware does not currently verify the SMTP server's certificate. Use the provider's official SMTP hostname and a trusted network connection.
...
Email providers impose sending limits and acceptable-use policies that vary by account type and may change over time. Check the current [Gmail sending limits](https://support.google.com/mail/answer/22839) or [Zoho Mail rates and limits](https://www.zoho.com/mail/help/adminconsole/rates-and-limits.html), and confirm that your account permits automated notifications.
...
![App Password entry](images/appPassEntry.png){ .img-border .img-center }
...
* If you prefer a non-Gmail account, Zoho Mail is an alternative. Begin by navigating to [this site](https://www.zoho.com/mail/?ireft=nhome&src=fa) to create a Zoho Mail **personal account**. Make sure to select **Personal Email**, not the default **Business Email**.

![Zoho Create Account screen](images/zohoCreateAccount.png){ .img-border .img-center }
...
![Zoho App Password entry](images/zohoAppPassEntry.png){ .img-border .img-center }
...
![Email Settings input](images/emailSettingsInput.png){ .img-border .img-center }
...
* **Sensor 3 Update** and **Sensor 4 Update**: The same as above on OpenSprinkler v3.4 and v4.
...
For Bundle Station runs, only the bundle leader generates Station Start and Station Finish notifications; derived bundle members do not generate separate messages.

!!! warning
    If your controller runs many zones daily, consider disabling high-frequency events such as **Station Start** and **Station Finish**. This helps avoid provider sending limits. Because each email connection takes time, **too many notification events can cause delays, missed responses, and skipped short watering cycles**.
