...
OpenSprinkler can send notification events to [IFTTT](https://ifttt.com/) through the IFTTT Webhooks service. An IFTTT Applet can then forward those events as push notifications, email, SMS, or another action supported by IFTTT.
...
2. Open **Explore**, search for **Webhooks**, and select the Webhooks service.
    ...
    ![Search for Webhooks in IFTTT](images/ifttt-webhooks-search.png){ .img-border .img-center width="600" }

    ![Select the Webhooks service](images/ifttt-webhooks-service.png){ .img-border .img-center width="500" }

3. Connect the Webhooks service if necessary. In its **About Webhooks** section, select **Documentation**.

    ![Open the Webhooks documentation](images/ifttt-webhooks-documentation.png){ .img-border .img-center width="550" }

4. Copy the Webhooks key shown on the documentation page. Treat this key as a password and do not publish it.

    ![Webhooks key on the documentation page](images/ifttt-webhooks-key.png){ .img-border .img-center width="650" }
...
2. For **If This**, select **Webhooks**, then choose **Receive a web request**.
    ...
    ![Choose the Webhooks trigger](images/ifttt-create-trigger.png){ .img-border .img-center width="650" }

3. Enter `sprinkler` as the **Event Name**, then create the trigger. This name is fixed by the OpenSprinkler firmware and is case-sensitive.

    ![Set the Webhooks event name to sprinkler](images/ifttt-event-name.png){ .img-border .img-center width="650" }
...
5. In the action's message field, insert the **Value1** ingredient. OpenSprinkler places the complete notification message in `Value1`.
    ...
    ![Insert Value1 into the IFTTT action](images/ifttt-value1.png){ .img-border .img-center width="650" }
...
3. Enter a descriptive **Device Name**. It is included in IFTTT and email notification messages and helps distinguish multiple controllers.
...
!!! warning
    Avoid enabling more events than you need, especially **Station Start** and **Station Finish** on controllers that run many zones. Sending each remote notification takes time; excessive notifications can cause delays, missed responses, or skipped short watering events. SMS carriers may also filter frequent messages.

For Bundle Station runs, only the bundle leader generates Station Start and Station Finish notifications. Derived bundle members do not generate separate notifications.
