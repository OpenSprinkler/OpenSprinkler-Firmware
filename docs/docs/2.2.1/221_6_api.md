...
* <span class="hl">**Bundle Zones**: A physical zone can now activate a configured set of additional Standard physical zones. Bundle configuration is exposed through the existing `/je` and `/cs` special-station fields, while `/jc`, `/jn`, and `/js` expose applied and bundle-derived state.</span>

### Changes Introduced in Firmware 2.2.1(5)
...
|<span class="hl">`bap`</span>| <span class="hl">Bundle-applied station bitfields, using the same layout as `sbits`. A set bit means an applied output is currently claimed by at least one Bundle Zone. A station may have both a direct queue entry and a bundle claim.</span>|
|`ps`   | Program status array with one `[pid, rem, start, gid]` entry per station. `pid` is `0` when idle, normally the 1-based program index, `99` for a manual run, or `254` for a run-once program. `rem`: the remaining runtime; `start`: the scheduled start time; `gid`: the sequential group ID. If the corresponding `sbit=0` while `pid!=0`, the station is queued but not running. For a short Bundle Zone run extended to complete member staggering, `rem` reflects the effective extended runtime; the duration saved in the program is unchanged.|
...
| `otc`     | OpenThings Cloud (OTC) configuration as JSON. The outer `{}` are optional. Example: `otc="en":1,"token":"token","server":"ws.cloud.openthings.io","port":80`|
...
| <span class="hl">`stn_bnd`</span> | <span class="hl">Bundle Zone leader bitfields. A set bit means the physical zone has Bundle Zone configuration (`st=7`).</span>|
| <span class="hl">`bmt`</span> | <span class="hl">Bundle member type capability mask. Bit `n` corresponds to special-station type `n` from `/je`; a set bit means that type may be selected as a Bundle Zone member. The current value is `1`, allowing only Standard stations (`st=0`). This is a 32-bit capability field; bits `8`-`31` are reserved.</span>|
...
| <span class="hl">`7`</span> | <span class="hl">Bundle Zone</span> | <span class="hl">Hexadecimal member bitmap with two characters per 8-zone group. Bytes are ordered by board and bit 0 represents the first zone in each group. For an 8-zone controller, `0E` selects zones 2, 3, and 4. The leader itself is not included. Eligible member types are advertised by `/jn.bmt`.</span> |
...
| `sd` | Special-station data payload (see `/je`). URL-encoded. For <span class="hl">Bundle Zones (`st=7`), this must contain exactly two hexadecimal characters per 8-zone group.</span>|
...
* <span class="hl">A Bundle Zone leader or member cannot be a master station. A member must be a Standard station (`st=0`) and cannot be the leader itself. Disabled members remain configured but are not activated. The allowed member types are advertised by `/jn.bmt`; clients should use that capability instead of hard-coding the current restriction.</span>
* <span class="hl">Changing a station referenced by any Bundle Zone to a special-station type, including Bundle Zone, returns Not Permitted (`48`). Remove it from every bundle first.</span>
...
* <span class="hl">`os-ip/cs?pw=xxx&sid=0&st=7&sd=0E`: configure zone 1 as a Bundle Zone that also activates zones 2, 3, and 4 on an 8-zone controller.</span>
...
| <span class="hl">`bap`</span> | <span class="hl">Bundle-applied bitfields. Each byte covers eight zones using the same LSB-first layout as `/jc.sbits`. A set bit means the applied output is currently claimed by a Bundle Zone.</span> |
| `nstations` | Total number of stations.                    |

<br>**Example:** `{"sn":[1,1,0,0,0,0,0,0],"bap":[2],"nstations":8}`
<br>Stations 1 and 2 (at indices `0` and `1` in the array) are currently open; station 2 is also claimed by a Bundle Zone.
...
* <span class="hl">A zone activated only as a Bundle Zone member has no individual runtime queue entry. Calling `/cm?en=0` for that member returns Out of Range (`17`), just as it does for an inactive zone. Stop the owning Bundle Zone leader to remove its claim. If the member also has a direct run, stopping it removes only that direct claim; the physical output remains active while any bundle still claims it.</span>
