...
| <span class="hl">`cursor`</span> | <span class="hl">Opaque pagination token from the preceding response's `X-OS-Next-Cursor` header. Omit on the first page.</span> |
...
<span class="hl">Binary mode returns zero or more 16-byte little-endian records. It may use `hist=all` or a `start`/`end` range longer than 365 days because each request is bounded by `count`. Repeat the request using `X-OS-Next-Cursor` exactly as returned, and stop only when `X-OS-Page-Done` is `1`.</span>
