...
| <span class="hl">`action`</span> | <span class="hl">Weather</span> | <span class="hl">Weather-action index advertised by `/jsd` in `enums.WeatherAction`. Default: `0` (Current Temperature).</span> |
...
```text
cm?pw=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx&sid=0&en=1&t=300&qo=2
```
...
On ESP8266, the MQTT library has a 256-byte packet limit. The MQTT topic and protocol overhead count toward this limit, so the available payload length is less than 256 bytes and varies with the topic length. Commands that exceed the limit are silently discarded by the MQTT library before the firmware receives them. This primarily affects `/cr` payloads containing durations for many stations. Use the HTTP API for commands that do not fit in one MQTT packet.
...
<br><span class="hl"><code>http://os-ip/jl?**pw**=xxx&**hist**=all&**fmt**=binary&**page**=1&**cursor**=xxx&**count**=n</code></span>

|Parameter |Meaning |
|:---------|:-------|
| `start`  | Start time (epoch seconds), inclusive.|
| `end`    | End time (epoch seconds), inclusive. Max span: `365` days.|
| `hist`   | History window in days back from today. `0`: today only; `1`: today and yesterday; and so on. Max span: `365` days.|
...
| <span class="hl">`cursor`</span> | <span class="hl">Opaque pagination token from the preceding response's `X-OS-Next-Cursor` header. Omit on the first page. The 2.2.1(6) token is 12 hexadecimal characters, but clients should not parse or modify it.</span> |
| <span class="hl">`count`</span> | <span class="hl">Maximum physical log slots scanned in one binary page. Default: `100`; maximum: `2500`. Deleted or filtered slots count toward this limit but are not returned.</span> |
...
<span class="hl">Binary mode returns zero or more 16-byte little-endian records from the bounded sprinkler-log ring. It may use `hist=all` or a `start`/`end` range longer than 365 days because each request is bounded by `count`. Repeat the request using `X-OS-Next-Cursor` exactly as returned, and stop only when `X-OS-Page-Done` is `1`.</span>
...
<span class="hl">Response headers:</span>
...
<span class="hl">Binary record layout:</span>
...
* <span class="hl">`os-ip/dl?pw=xxx&before=20000`: delete Sprinkler Log records older than day `20000`.</span>
