...
#error setCACert not implemented
        ...
        if(host && _client.tcp->connect((uint8_t*)host->h_addr, _port)) {
#endif
            connectedCb();
            _lastConnectionFail = 0;
        } else {
...
void WebSocketsClient::handleClientData(void) {
    ...
    if(len > 0) {
        switch(_client.status) {
            ...
            case WSC_BODY: {
                char buf[256] = { 0 };
                len = _client.tcp->readBytes(&buf[0], sizeof(buf));
                buf[len] = 0;
                String bodyLine = buf;
...
void WebSocketsClient::sendHeader(WSclient_t * client) {
    ...
    handshake += url + WEBSOCKETS_STRING(
                           " HTTP/1.1\r\n"
                           "Host: ");
    handshake += _host + ":" + std::to_string(_port) + NEW_LINE;
...
void WebSocketsClient::handleHeader(WSclient_t * client, String * headerLine) {
    ...
    if(headerLine->length() > 0 && client->isSocketIO && client->status == WSC_BODY && client->cSessionId.length() == 0) {
        ...
        if(indexOf(headerLine, sid_begin) > -1) {
            int start          = indexOf(headerLine, sid_begin) + sid_begin.length();
            int end            = indexOf(headerLine, "\"", start);
            client->cSessionId = substring(headerLine, start, end);
    ...
    if(headerLine->length() > 0) {
        ...
        if(startsWith(headerLine, WEBSOCKETS_STRING("HTTP/1."))) {
            // "HTTP/1.1 101 Switching Protocols"
            client->cCode = stoi(substring(headerLine, 9, indexOf(headerLine, ' ', 9)));
        } else if(indexOf(headerLine, ':') >= 0) {
            String headerName  = substring(headerLine, 0, indexOf(headerLine, ':'));
            String headerValue = substring(headerLine, indexOf(headerLine, ':') + 1);

            // remove space in the beginning  (RFC2616)
            if(headerValue[0] == ' ') {
                remove(headerValue, 0, 1);
            }

            if(equalsIgnoreCase(headerName, WEBSOCKETS_STRING("Connection"))) {
                if(equalsIgnoreCase(headerValue, WEBSOCKETS_STRING("upgrade"))) {
                    client->cIsUpgrade = true;
                }
            } else if(equalsIgnoreCase(headerName, WEBSOCKETS_STRING("Upgrade"))) {
                if(equalsIgnoreCase(headerValue, WEBSOCKETS_STRING("websocket"))) {
                    client->cIsWebsocket = true;
                }
            } else if(equalsIgnoreCase(headerName, WEBSOCKETS_STRING("Sec-WebSocket-Accept"))) {
                client->cAccept = headerValue;
                trim(client->cAccept);    // see rfc6455
            } else if(equalsIgnoreCase(headerName, WEBSOCKETS_STRING("Sec-WebSocket-Protocol"))) {
                client->cProtocol = headerValue;
            } else if(equalsIgnoreCase(headerName, WEBSOCKETS_STRING("Sec-WebSocket-Extensions"))) {
                client->cExtensions = headerValue;
            } else if(equalsIgnoreCase(headerName, WEBSOCKETS_STRING("Sec-WebSocket-Version"))) {
                client->cVersion = stoi(headerValue);
            } else if(equalsIgnoreCase(headerName, WEBSOCKETS_STRING("Set-Cookie")) && indexOf(headerValue, " io=") > -1) {
                if(indexOf(headerValue, ';') > -1) {
                    client->cSessionId = substring(headerValue, indexOf(headerValue, '=') + 1, indexOf(headerValue, ";"));
                } else {
                    client->cSessionId = substring(headerValue, indexOf(headerValue, '=') + 1);
                }
            }
        } else {
